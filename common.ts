...
import { readableStreamToString, NotionError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new NotionError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new NotionError(e, e.response?.data)
                }
                if (innerError instanceof NotionError) {
                    // Got "NotionError" from the above try block
                    throw innerError;
                }
